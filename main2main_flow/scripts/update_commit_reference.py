...
def run_update(ascend_path: Path, old_commit: str, new_commit: str) -> dict:
    """Replace old_commit with new_commit across all tracked files in ascend_path.

    Returns a dict with files_updated and replacement_count.
    Raises ValueError if old_commit is not found in any tracked file.
    """
    old_commit = old_commit.lower()
    new_commit = new_commit.lower()
    updated = _replace_in_tracked_files(ascend_path, old_commit, new_commit, dry_run=False)
    if not updated:
        raise ValueError(
            f"old commit {old_commit} not found in any tracked file in {ascend_path}"
        )
    return {
